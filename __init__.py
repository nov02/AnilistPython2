...
class Anilist:
    ...
    def getAnimeID(self, anime_name):
        ...
        if counter == 0:
            ...
            user_input = 1
            inp.append(data['data']['Page']['media'][user_input - 1]['id'])
            return inp # single input as a list
        multidata = []
        for i in range(1, max_result+1):
            inp.append(i)
        for input in inp:
            multidata.append(data['data']['Page']['media'][input - 1]['id'])
        return multidata
